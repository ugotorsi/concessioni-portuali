...
describe("B2C9 Block 3B.4A-R1 classification handoff", () => {
  it("contains no provider, research, canonical source, or async-job side effect", () => {
    const source = readFileSync(resolve("src/server/intake/classification/handoff.ts"), "utf8");
    expect(source).not.toMatch(/openai|provider.*invoke|legalResearch|admitAsyncJob|\.legalSource\.create/i);
  });
